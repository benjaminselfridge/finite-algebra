...
instance Eq a => Algebra (Group a) a GroupLaw where
  ...
  algebraLaws g = [ (IdClosed       , Property $ gId g `elem` algebraSet g)
                  , (InvClosed      , Property $ \a -> gInv g a `elem` algebraSet g)
                  , (MulClosed      , Property $ \a b -> gMul g a b `elem` algebraSet g)
                  , (IdLeftIdentity , Property $ \a -> gMul g (gId g) a == a)
                  , (IdRightIdentity, Property $ \a -> gMul g a (gId g) == a)
                  , (InvLeftInverse , Property $ \a -> gMul g (gInv g a) a == gId g)
                  , (InvRightInverse, Property $ \a -> gMul g a (gInv g a) == gId g)
                  , (MulAssoc       , Property $ \a b c -> gMul g (gMul g a b) c == gMul g a (gMul g b c))
                  ]
...
instance (Eq a, Eq b) => Morphism (GroupHomomorphism a b) GroupHomomorphismLaw (Group a) (Group b) a b GroupLaw where
  ...
  morphism = ghMap
  morphismLaws h = [ (GHClosed
                     , Property $ \a -> ghMap h a `elem` gSet (ghCodomain h)
                     )
                   , ( GHRespectsMultiplication
                     , Property $ \a b -> ghMap h (gMul (ghDomain h) a b) == gMul (ghCodomain h) (ghMap h a) (ghMap h b)
                     )
                   ]
